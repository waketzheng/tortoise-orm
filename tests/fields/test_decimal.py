...
async def test_scale_is_preserved(db):
    ...
    obj = await testmodels.DecimalFields.get(id=obj0.id)
    assert str(obj.decimal) == "1E+2"
    assert f"{obj.decimal:f}" == "100"
    assert str(obj.decimal_nodec) == "1E+1"
    assert f"{obj.decimal_nodec:f}" == "10"
    assert obj.decimal.as_tuple().exponent == 2
    assert obj.decimal_nodec.as_tuple().exponent == 1
...
def test_to_python_value_scale():
    ...
    assert str(field0.to_python_value("10.0000000")) == "1E+1"
    assert f"{field0.to_python_value('10.0000000'):f}" == "10"
    assert str(field0.to_python_value("200")) == "2E+2"
    assert f"{field0.to_python_value('200'):f}" == "200"
